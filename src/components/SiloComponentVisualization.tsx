...
const SiloComponentVisualization: FC<SiloComponentVisualizationProps> = ({
  ...
  const getComponentIcon = (type: SiloComponent["type"], color: string, status?: ComponentStatus, componentId?: string) => {
    ...
    switch (type) {
      ...
      case "transferidor":
        return (
          <div className={`relative ${componentColor} ${animationClass}`} style={{ width: '45px', height: '45px' }}>
            <svg 
              width="30" 
              height="30" 
              viewBox="0 0 100 70" 
      ...
      case "puente":
        return (
          ...
          >
            <Link to="/puente" className="no-underline">
              <span className="absolute -top-3 left-1/2 transform -translate-x-1/2 text-xs font-semibold text-gray-700 bg-white/80 rounded px-1 shadow-sm hover:bg-gray-200 cursor-pointer">PT</span>
            </Link>
            <svg 
              width="30" 
              height="30" 
  ...
  const getAbsolutePosition = (component: SiloComponent) => {
    ...
    if (component.type === "elevador") {
      ...
      return {
        ...
        top: `72%`,           // Ajustado ligeramente hacia arriba
            ...
            <TooltipTrigger asChild>
              ...
              >
                <div className="relative">
                  ...
                  <span
                    className={`absolute w-3 h-3 rounded-full border border-white shadow-sm ${statusColor}`}
                    style={component.type === "elevador" ? { left: '45px', top: '5px' } : 
                           component.type === "puente" ? { left: '40px', top: '0px' } : 
                           { left: '25px', top: '5px' }}
                  />
                ...
                {/* Mostrar etiquetas para carro transferidor y elevador debajo */}
                {(component.type === "transferidor" || component.type === "elevador") && (
                  <Link 
                    to={
                    ...
                    style={component.type === "elevador" ? { position: 'absolute', bottom: '60px' } : undefined}
                  >
            ...
            <TooltipContent>
              <div>
                ...
                {component.type === "puente" && ptData && (
                  <div className="space-y-1 mt-1">
                    <div>Estado: <span className="font-mono">{ptData.pt_estado}</span></div>
                    <div>Ocupación: <span className="font-mono">{ptData.pt_ocupacion}</span></div>
                    <div>Situación: <span className="font-mono">{ptData.pt_situacion}</span></div>
                    <div>Posición: <span className="font-mono">{ptData.pt_posicion}</span></div>
                  </div>
                )}
